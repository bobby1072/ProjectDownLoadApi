...
export default abstract class Routes {
  ...
  public static async FishLoggerDownload(app: Application) {
    app.get("/fish-logger", async (req: Request, res: Response) => {
      try {
        const userAgent = req.headers["user-agent"];
        ...
        res.status(200);
        res.download(
          `./src/applications/fish-logger/fish-logger-${devTypeAndExtension.deviceTypeAb}-setup.${devTypeAndExtension.extension}`
        );
      } catch (e) {
        let message = Constants.InternalServerError;
